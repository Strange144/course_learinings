function UserDetails({ user, theme, action }) {
    return (
        <div>
            <div className={`${theme.backgroundColor} ${theme.textColor} `}>
                <p><span>{user.avatar}</span>Name : {user.name}</p>
                <p>{user.role}</p>
...
export default function ComplexProps() {
    const users = [
        {
            ...
            actions: {
                primary: {
                    label: "View Profile",
                    onCComplexPropslick: () => setMessage("Viewing Alice's profile"),
    ...
    return (
        <>
            <h1 className="text-white">ComplexProps</h1>
